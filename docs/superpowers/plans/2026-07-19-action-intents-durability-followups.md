...
```
Expected: `intentFanout.integration.test.ts` passes (autoMigrate runs once in globalSetup, fan-out assertions green). If it fails on connection/migration, resolve the harness before proceeding (see `docs/superpowers/plans/2026-04-11-rls-coverage-gaps.md` and memory `test_integration_config_run_mechanics`).
