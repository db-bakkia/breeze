...
gh pr create --title "chore: upgrade zod to v4 (2/2)" \
  --body "Stage 2: flips zod to ^4 and fixes the v4-only hard-breaks (required_error/invalid_type_error -> error). Builds on the pre-fixes in PR 1. Full typecheck + shared/web unit + API integration/RLS suites green. Plan: docs/superpowers/plans/2026-06-16-zod-3-to-4-upgrade.md"
```
